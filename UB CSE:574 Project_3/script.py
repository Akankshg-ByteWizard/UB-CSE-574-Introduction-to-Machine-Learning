...
def blrObjFunction(initialWeights, *args):
    ...
    a = 1
    DAT = np.full((n_data,1),a)
    X = np.concatenate((DAT,train_data),axis=1)
    column = 1
    n_row = n_features + 1
    
    w = initialWeights.reshape(n_row,column)
    theta_value = sigmoid(X@w)
    pro_1 = labeli * np.log(theta_value)
    pro_2 = (1.0 - labeli) * np.log(1.0 - theta_value)
    
    total = pro_1 + pro_2
    fxnerror = total
    error = ( -1.0 * np.sum(fxnerror) ) / n_data
...
def blrPredict(W, data):
    ...
    a = 1
    DAT = np.full((N,1),a)
    X = np.concatenate((DAT,data),axis=1)
    pro_3 = sigmoid(X@W)
    l = np.argmax(pro_3,axis=1)
    label = l.reshape((N,1))
...
def mlrObjFunction(params, *args):
    ...
    bot = np.sum(top,axis=1)
    bot = bot.reshape(bot.shape[0],a)
    
    theta_value = top/bot
    
   
    Sum_I = np.sum(Y*np.log(theta_value))

    error = - (np.sum(Sum_I))
    o = (X.T @ (theta_value - labeli))
    error_grad = o
...
def mlrPredict(W, data):
    ...
    t = np.sum(np.exp(X@W),axis=1)    
    t = t.reshape(t.shape[0],a)
    
    theta_value = np.exp(X@W)/t
    
    label = np.argmax(theta_value,axis=1)
    label = label.reshape(row,a)
...
# C values
for c in C_values:
    print("C Value: {}/n".format(c))
    rbf_mod2 = svm.SVC(kernel = 'rbf', C = c)
    rbf_mod2.fit(trsvm_data, trsvm_label.ravel())
    
    if input <= 10: 
        accuracy[input][0] = 100 * rbf_mod2.score(train_data, train_label)
        accuracy[input][1] = 100 * rbf_mod2.score(validation_data, validation_label)
        accuracy[input][2] = 100 * rbf_mod2.score(test_data, test_label)
        
        print('\n---------RBF Kernel with Gamma = default and C = '+ str(c) +'---------\n')
        print('\n Training Accuracy -->' + str(accuracy[input][0]) + '%')
        print('\n Validation Accuracy -->' + str(accuracy[input][1]) + '%')
        print('\n Testing Accuracy -->' + str(accuracy[input][2]) + '%')
